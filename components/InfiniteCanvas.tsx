...
const OutpaintingFrame: React.FC<OutpaintingFrameProps> = ({ outpaintingState, zoom, onUpdateFrame }) => {
    ...
    const handleInteractionStart = useCallback((e: React.MouseEvent | React.TouchEvent, type: NonNullable<typeof interactionRef.current>['type']) => {
        e.stopPropagation();
...
interface InfiniteCanvasProps {
  ...
  onUpdateMultipleElements: (updates: (Partial<CanvasElement> & { id: string })[]) => void;
  onInteractionEnd: () => void;
  setResetViewCallback: (callback: () => void) => void;
  onGenerate: (selectedElements: CanvasElement[]) => void;
  onContextMenu: (e: React.MouseEvent | React.TouchEvent, worldPoint: Point, elementId: string | null) => void;
...
export const InfiniteCanvas = forwardRef<CanvasApi, InfiniteCanvasProps>(({ 
  ...
  const [groupInteraction, setGroupInteraction] = useState<GroupInteraction>(null);
  ...
  const zoomToPoint = useCallback((newZoom: number, screenPoint: Point) => {
  ...
  const zoomIn = useCallback(() => {
    ...
    const centerScreen = { x: canvasRef.current.clientWidth / 2, y: canvasRef.current.clientHeight / 2 };
  ...
  const panTo = useCallback((worldPoint: Point) => {
    if (!canvasRef.current) return;
  ...
  useEffect(() => {
    if (onViewportChange && canvasRef.current) {
      ...
      const worldBottomRight = screenToWorld({ x: clientWidth, y: clientHeight });
      onViewportChange({
        x: worldTopLeft.x,
        y: worldTopLeft.y,
  ...
  const performMarqueeSelection = useCallback((rect: MarqueeRect, shiftKey: boolean) => {
    if (!rect) return;
  ...
  const handleMouseDown = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    const isInteractingWithElement = (e.target as HTMLElement).closest('.transform-handle, .element-body, .generate-controls, .analysis-controls');
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<HTMLDivElement>) => {
    ...
    if (isPanning) {
      ...
      const newRect = { ...marqueeRectRef.current, end: { x: e.clientX, y: e.clientY } };
      setMarqueeRect(newRect);
      marqueeRectRef.current = newRect;
    } else if (groupInteraction) {
        const { type, startPoint, startElements, startBbox, center } = groupInteraction;
        const currentPoint = { x: e.clientX, y: e.clientY };
        
        if (type === 'group-rotate' && groupInteraction.startAngle !== undefined) {
             const radPerPixel = 1 / (100 * zoom); // sensitivity
             const currentAngle = Math.atan2(currentPoint.y - (startBbox.minY * zoom + pan.y - 16), currentPoint.x - (center.x * zoom + pan.x));
             const startAngle = Math.atan2(startPoint.y - (startBbox.minY * zoom + pan.y - 16), startPoint.x - (center.x * zoom + pan.x));
             const angleDiff = currentAngle - startAngle;

             const updates = startElements.map(el => {
                const rotatedPosition = {
                    x: center.x + (el.position.x - center.x) * Math.cos(angleDiff) - (el.position.y - center.y) * Math.sin(angleDiff),
                    y: center.y + (el.position.x - center.x) * Math.sin(angleDiff) + (el.position.y - center.y) * Math.cos(angleDiff),
                };
                return {
                    id: el.id,
            ...
            const originalDiagonal = Math.sqrt(startBbox.width ** 2 + startBbox.height ** 2);
            const newDiagonal = Math.sqrt((startBbox.width + dx) ** 2 + (startBbox.height + dy) ** 2);
            ...
            const updates = startElements.map(el => {
                ...
                const relativePos = { x: el.position.x - center.x, y: el.position.y - center.y };
                const newRelativePos = { x: relativePos.x * ratio, y: relativePos.y * ratio };
                const newPosition = { x: center.x + newRelativePos.x, y: center.y + newRelativePos.y };

                return { id: el.id, width: newWidth, height: newHeight, position: newPosition };
            });
            onUpdateMultipleElements(updates);
        }
    }
  ...
  const handleTouchStart = useCallback((e: TouchEvent) => {
    ...
    if (target.closest('.transform-handle, .element-body, .generate-controls, .analysis-controls')) {
        return;
    }
    e.preventDefault();
    ...
    if (touches.length === 1 && startPoint) {
        ...
        if (!touchStateRef.current.didMove) {
             ...
             if (dist > 5) {
                 ...
                 if (interactionMode === 'select' && !outpaintingState && !marqueeRectRef.current) {
                    touchStateRef.current.isMarquee = true;
                    onSelectElement(null, false);
                    const newRect = { start: startPoint, end: { x: touch.clientX, y: touch.clientY } };
                    setMarqueeRect(newRect);
                    marqueeRectRef.current = newRect;
                 }
             }
        ...
        if (touchStateRef.current.didMove) {
            if (touchStateRef.current.isMarquee) {
                 const newRect = { ...marqueeRectRef.current!, end: { x: touch.clientX, y: touch.clientY } };
                 setMarqueeRect(newRect);
                 marqueeRectRef.current = newRect;
        ...
        if (lastDist > 0) {
            ...
            const pinchCenterX = (currT1.clientX + currT2.clientX) / 2 - rect.left;
            const pinchCenterY = (currT1.clientY + currT2.clientY) / 2 - rect.top;

            const newZoom = Math.max(MIN_ZOOM, Math.min(MAX_ZOOM, zoom * (currDist / lastDist)));
  ...
  }, [pan, zoom, interactionMode, onSelectElement, outpaintingState, onMouseMove, screenToWorld, zoomToPoint]);
  ...
  const handleTouchEnd = useCallback((e: React.TouchEvent<HTMLDivElement>) => {
    ...
    } else if (!touchStateRef.current.didMove && touchStateRef.current.startPoint) {
        onSelectElement(null, false);
    }
    ...
    touchStateRef.current.lastTouches = e.touches.length > 0 ? Array.from(e.touches) : null;
    touchStateRef.current.startPoint = null;
    touchStateRef.current.didMove = false;
    touchStateRef.current.isMarquee = false;
  }, [onSelectElement, performMarqueeSelection, groupInteraction, onInteractionEnd]);
  ...
  const selectionBbox = useMemo((): BoundingBox | null => {
      ...
      return { minX, minY, maxX, maxY, width: maxX - minX, height: maxY - minY, rotation: 0 };
  }, [elements, selectedElementIds]);

    const handleGroupInteractionStart = useCallback((e: React.MouseEvent | React.TouchEvent, type: GroupInteraction['type']) => {
        ...
        const startElements = elements.filter(el => selectedElementIds.includes(el.id));
        ...
        if (type === 'group-rotate') {
            ...
            interactionDetails.startAngle = Math.atan2(startPoint.y - bboxCenterScreen.y, startPoint.x - bboxCenterScreen.x);
        }
  ...
  return (
    ...
    >
        ...
        {sortedElements.map((el) => (
          <TransformableElement
            ...
            onContextMenu={(e, elementId) => {
              const point = 'touches' in e ? e.touches[0] : e;
              const worldPoint = screenToWorld({x: point.clientX, y: point.clientY});
        ...
        {selectionBbox && (
             <div className="absolute pointer-events-none">
                 ...
                 {selectedElementIds.length > 1 && !outpaintingState && (
                    <>
                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 w-5 h-5 bg-blue-500 rounded-full cursor-alias group-transform-handle"
                            style={{
                                left: selectionBbox.minX + selectionBbox.width / 2,
                                top: selectionBbox.minY,
                                transform: `translate(-50%, -150%)`,
                            }}
                            onMouseDown={(e) => handleGroupInteractionStart(e, 'group-rotate')}
                            onTouchStart={(e) => handleGroupInteractionStart(e, 'group-rotate')}
                        />
                         <div className="absolute left-1/2 -translate-x-1/2 w-0.5 h-3 bg-blue-500 pointer-events-none"
                            style={{
                                left: selectionBbox.minX + selectionBbox.width / 2,
                                top: selectionBbox.minY,
                        ...
                        <div className="absolute w-4 h-4 bg-white border-2 border-blue-500 rounded-sm cursor-se-resize group-transform-handle"
                             style={{
                                left: selectionBbox.maxX,
                                top: selectionBbox.maxY,
                                transform: `translate(-50%, -50%)`,
                            }}
                            onMouseDown={(e) => handleGroupInteractionStart(e, 'group-resize')}
                            onTouchStart={(e) => handleGroupInteractionStart(e, 'group-resize')}
                        />
                    </>
                 )}
             </div>
      ...
      {outpaintingState && (
        ...
        >
          <h3 className="text-sm font-semibold text-gray-700">{t('expandImage')}</h3>
          ...
          <div className="flex gap-2">
              <button onClick={handleOutpaintingCancelClick} className="flex-1 px-3 py-1.5 text-sm bg-gray-200 text-gray-800 rounded-md hover:bg-gray-300">{t('cancel')}</button>
              <button onClick={handleOutpaintingGenerateClick} className="flex-1 px-3 py-1.5 text-sm bg-teal-600 text-white rounded-md hover:bg-teal-700 whitespace-nowrap">{t('generate')} ✨</button>
          </div>
        </div>
      )}
