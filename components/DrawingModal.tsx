...
type Tool = 'pencil' | 'eraser' | 'line' | 'rectangle' | 'circle' | 'triangle' | 'star' | 'arrow';

// FIX: Changed JSX.Element to React.ReactElement to resolve "Cannot find namespace 'JSX'" error.
const SHAPE_TOOLS: { name: Tool; icon: React.ReactElement }[] = [
    { name: 'line', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="19" x2="19" y2="5"></line></svg> },
    { name: 'rectangle', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect></svg> },
    { name: 'circle', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"></circle></svg> },
    { name: 'triangle', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2L2 22h20L12 2z"></path></svg> },
    { name: 'star', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path></svg> },
    { name: 'arrow', icon: <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="5" y1="12" x2="19" y2="12"></line><polyline points="12 5 19 12 12 19"></polyline></svg> },
];
...
const drawStar = (ctx: CanvasRenderingContext2D, start: Point, end: Point, points = 5) => {
...
export const DrawingModal: React.FC<DrawingModalProps> = ({ element, onSave, onClose, t }) => {
  ...
  const startDrawing = useCallback((e: React.MouseEvent | React.TouchEvent) => {
    if (('button' in e && e.button !== 0) || ('touches' in e && e.touches.length > 1)) {
        return;
    }
    e.preventDefault();
  ...
  const finishDrawing = useCallback(() => {
    ...
    if (isShapeTool) {
        ...
        if (shapeStartPointRef.current && shapeEndPointRef.current) {
            drawShape(context, tool, shapeStartPointRef.current, shapeEndPointRef.current);
        }
    } else {
  ...
  const drawShape = (ctx: CanvasRenderingContext2D, shape: Tool, start: Point, end: Point) => {
        ...
        switch (shape) {
            ...
            case 'circle': drawCircle(ctx, start, end); break;
            case 'triangle': drawTriangle(ctx, start, end); break;
            case 'star': drawStar(ctx, start, end); break;
      ...
      <div className="bg-white rounded-lg shadow-2xl w-full max-w-7xl h-[95vh] flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-2 border-b flex flex-wrap items-center gap-4 bg-gray-100 flex-shrink-0">
            ...
            <div className="flex items-center gap-2">
                <button onClick={() => handleToolSelect('pencil')} title={t('pencil')} className={`p-2 rounded ${tool === 'pencil' ? 'bg-blue-500 text-white' : 'bg-white text-gray-800 hover:bg-gray-200'}`}><svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z"></path></svg></button>
                <button onClick={() => handleToolSelect('eraser')} title={t('eraser')} className={`p-2 rounded ${tool === 'eraser' ? 'bg-blue-500 text-white' : 'bg-white text-gray-800 hover:bg-gray-200'}`}><svg viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" strokeWidth="2" fill="none" strokeLinecap="round" strokeLinejoin="round"><path d="M20.49 4.51a2.828 2.828 0 0 0-4-4L5 11.51l-4 4.5 1.5 1.5 4.5-4L20.49 4.51zM11 5l9 9"></path></svg></button>
            </div>
            {/* Shapes */}
            <div className="flex items-center gap-2 pl-2 border-l border-gray-300">
                <span className="text-sm font-medium text-gray-700">{t('shapes')}:</span>
                {SHAPE_TOOLS.map(({name, icon}) => (
                    <button key={name} onClick={() => handleToolSelect(name)} title={t(name)} className={`p-2 rounded ${tool === name ? 'bg-blue-500 text-white' : 'bg-white text-gray-800 hover:bg-gray-200'}`}>
            ...
            {/* Undo/Redo */}
            <div className="flex items-center gap-2 pl-2 border-l border-gray-300">
                <button onClick={undo} disabled={!canUndo} className={`p-2 rounded ${!canUndo ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-white text-gray-800 hover:bg-gray-200'}`}>{t('undo')}</button>
                <button onClick={redo} disabled={!canRedo} className={`p-2 rounded ${!canRedo ? 'bg-gray-200 text-gray-500 cursor-not-allowed' : 'bg-white text-gray-800 hover:bg-gray-200'}`}>{t('redo')}</button>
            </div>
            {/* Brush Size */}
            <div className="flex items-center gap-2">
