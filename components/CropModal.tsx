...
interface CropModalProps {
  ...
  onSave: (elementId: string, dataUrl: string, newWidth: number, newHeight: number) => void;
  onClose: () => void;
  t: (key: string) => string;
}

export const CropModal: React.FC<CropModalProps> = ({ element, onSave, onClose, t }) => {
  ...
  const handleMove = useCallback((e: MouseEvent | TouchEvent) => {
    ...
    const dx = ((pos.clientX - startPos.current.x) / containerSize.width) * 100;
    const dy = ((pos.clientY - startPos.current.y) / containerSize.height) * 100;
    ...
    setCropRect(prev => {
      ...
      if (isDragging.current === 'move') {
        ...
        if (aspectRatio !== null && imgRef.current) {
             ...
             } else if (isDragging.current === 'ne' || isDragging.current === 'nw') {
             ...
             } else if (isDragging.current === 'e' || isDragging.current === 'w') {
        ...
        if (newWidth < minSize) {
             newWidth = minSize;
             if (isDragging.current === 'nw' || isDragging.current === 'sw' || isDragging.current === 'w') newX = x + width - minSize; 
             if (isDragging.current === 'n' || isDragging.current === 's' || isDragging.current === 'e' || isDragging.current === 'w') {
                // If it was a midpoint drag and hit min size, re-center logic might need correction, but simpler to just clamp width
                if (aspectRatio !== null && (isDragging.current === 'n' || isDragging.current === 's')) {
             ...
             if (aspectRatio !== null && imgRef.current) {
                ...
                if (isDragging.current === 'nw' || isDragging.current === 'ne') newY = y + height - newHeight;
        ...
        if (newHeight < minSize) {
             newHeight = minSize;
             if (isDragging.current === 'nw' || isDragging.current === 'ne' || isDragging.current === 'n') newY = y + height - minSize;
             
             if (aspectRatio !== null && imgRef.current) {
                 // If height was constrained, update width
                const imageRatio = imgRef.current.naturalWidth / imgRef.current.naturalHeight;
                newWidth = newHeight * (aspectRatio / imageRatio);
                if (isDragging.current === 'nw' || isDragging.current === 'sw') newX = x + width - newWidth;
  ...
  return (
    <div className="fixed inset-0 z-[100] bg-black/90 flex flex-col items-center justify-center p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden" onClick={e => e.stopPropagation()}>
          ...
          <div ref={containerRef} className="relative inline-block shadow-2xl">
            ...
            {imgLoaded && (
              <>
                ...
                >
                    {/* Corner Handles */}
                    <div className="absolute -top-1.5 -left-1.5 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-nw-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'nw')} onTouchStart={e => handleStart(e, 'nw')}></div>
                    <div className="absolute -top-1.5 -right-1.5 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-ne-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'ne')} onTouchStart={e => handleStart(e, 'ne')}></div>
                    <div className="absolute -bottom-1.5 -left-1.5 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-sw-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'sw')} onTouchStart={e => handleStart(e, 'sw')}></div>
                    <div className="absolute -bottom-1.5 -right-1.5 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-se-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'se')} onTouchStart={e => handleStart(e, 'se')}></div>
                    
                    {/* Midpoint Handles */}
                    <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-n-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'n')} onTouchStart={e => handleStart(e, 'n')}></div>
                    <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-s-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 's')} onTouchStart={e => handleStart(e, 's')}></div>
                    <div className="absolute top-1/2 -left-1.5 -translate-y-1/2 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-w-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'w')} onTouchStart={e => handleStart(e, 'w')}></div>
                    <div className="absolute top-1/2 -right-1.5 -translate-y-1/2 w-4 h-4 bg-white border border-gray-400 rounded-full cursor-e-resize shadow-sm z-20" onMouseDown={e => handleStart(e, 'e')} onTouchStart={e => handleStart(e, 'e')}></div>
        ...
        <div className="p-4 border-t border-gray-800 bg-[#0F172A] flex justify-between items-center gap-3">
          ...
          <div className="flex gap-2">
            <button onClick={onClose} className="px-4 py-2 text-sm font-medium bg-gray-700 text-gray-200 rounded-lg hover:bg-gray-600 transition-all">{t('cancel')}</button>
            <button onClick={handleSave} className="px-5 py-2 text-sm font-bold bg-blue-600 text-white rounded-lg hover:bg-blue-700 shadow-lg shadow-blue-900/20 transition-all flex items-center gap-2">
